...
/**
  ...
  July 19th, 2021.
*/
contract SuperEnjin1155 is PermitControl, ERC165Storage, IERC1155, IERC1155MetadataURI {
  ...
  */
  function safeBatchTransferFrom(address _from, address _to,
    ...
    for (uint256 i = 0; i < _ids.length; ++i) {
      ...
      // Check transfer type.
      if (itemGroups[groupId].transferData.transferType == Data.TransferType.BoundToAddress) {
        revert("Bound to Address");
      } else if (itemGroups[groupId].transferData.transferType == Data.TransferType.TemporaryTransfer) {
        require(block.timestamp <= itemGroups[groupId].transferData.transferTime, "Transfer time is over");
      }

      // Check transfer fee type.
      if (itemGroups[groupId].transferData.transferFeeType == Data.TransferFeeType.PerTransfer) {
        bool paid;
      ...
      } else if (itemGroups[groupId].transferData.transferFeeType == Data.TransferFeeType.PerItem) {
      ...
      } else if (itemGroups[groupId].transferData.transferFeeType == Data.TransferFeeType.RatioCut) {
        if (itemGroups[groupId].itemType == Data.ItemType.Fungible) {
      ...
      if (itemGroups[groupId].transferData.transferFeeType == Data.TransferFeeType.RatioCut) {
  ...
  */
  function configureGroup(uint256 _groupId, Data.ItemGroupInput calldata _data) external payable {
    require(_groupId != 0,
    ...
    // Edit an existing item group. The name may always be updated.
      ...
      if (itemGroups[_groupId].supplyType == Data.SupplyType.Capped) {
        require(_data.supplyType == Data.SupplyType.Capped,
      ...
      if (itemGroups[_groupId].itemType == Data.ItemType.Nonfungible) {
        require(_data.itemType == Data.ItemType.Nonfungible,
          "Super1155: you may not alter nonfungible items");

      // A semifungible item may not change type.
      } else if (itemGroups[_groupId].itemType == Data.ItemType.Semifungible) {
        require(_data.itemType == Data.ItemType.Semifungible,
          "Super1155: you may not alter nonfungible items");

      // A fungible item may change type if it is unique enough.
      } else if (itemGroups[_groupId].itemType == Data.ItemType.Fungible) {
  ...
  */
  function mintBatch(address _recipient, uint256[] calldata _ids,
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      ...
      } else if (itemGroups[groupId].supplyType == Data.SupplyType.TimePercent) {
        uint256 intervals = (block.timestamp - itemGroups[groupId].timeData.timeStamp) / itemGroups[groupId].timeData.timeInterval;
