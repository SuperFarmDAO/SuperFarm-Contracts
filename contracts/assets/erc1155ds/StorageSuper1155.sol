...
contract StorageSuper1155 {
    ...
    */
    constructor(address _linkProxy, string memory _name, string memory _metadataURI, string memory _contractURI, address _proxyRegistryAddress) {
        ...
        bytes32 linkProxyKey = keccak256(abi.encodePacked(selector, baseContractName)); // Relevant facet

        // Do a delegate call to initialize the owner
        address initializerFacet = ILinkProxy(_linkProxy).links(linkProxyKey);
        require(initializerFacet != address(0), "StorageSuper1155: No facet found!");
    ...
    fallback() external {
        ...
        string memory baseContractName = "Super1155";
        bytes32 linkProxyKey = keccak256(abi.encodePacked(msg.sig, baseContractName)); // Relevant facet
