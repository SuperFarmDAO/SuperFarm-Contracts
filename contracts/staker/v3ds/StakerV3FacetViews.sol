...
*/
contract StakerV3FacetViews is Sweepableds, ReentrancyGuard, ERC1155Holder, IERC721Receiver {
   ...
   */
  function getItemsUserInfo(address _itemUserAddress, uint256 _boosterId) external view returns(StakerV3Blueprint.GetItemUserInfo memory) {
    
    StakerV3Blueprint.StakerV3StateVariables
      storage b = StakerV3Blueprint.stakerV3StateVariables();

    uint256 length = b.itemUserInfo[_itemUserAddress].tokenIds[_boosterId].length();
    uint256[] memory _tokenIds = new uint256[](length);
    uint256[] memory _amounts = new uint256[](length);
    for (uint256 i = 0; i < length; i++) {
      _tokenIds[i] = b.itemUserInfo[_itemUserAddress].tokenIds[_boosterId].at(i);
      _amounts[i] = b.itemUserInfo[_itemUserAddress].amounts[_tokenIds[i]];
    }

    StakerV3Blueprint.GetItemUserInfo memory _userInfo = StakerV3Blueprint.GetItemUserInfo({
