    ...
    ERC1155Holder
{
  ...
  */
    ...
    {
        require(canAlterDevelopers, "Devs locked.");
  ...
  */
    function setEmissions(
        ...
        if (_tokenSchedule.length > 0) {
            require(canAlterTokenEmissionSchedule, "Token emissions locked.");
        ...
        require(tokenEmissionEventsCount > 0, "Set token emissions.");

        if (_pointSchedule.length > 0) {
            require(canAlterPointEmissionSchedule, "Point emissiosn locked.");
        ...
        require(pointEmissionEventsCount > 0, "Set point emissions.");
    }
  ...
  */
    function addPool(
    ...
    ) external hasValidPermit(UNIVERSAL, ADD_POOL) {
        require(
            tokenEmissionEventsCount > 0 && pointEmissionEventsCount > 0,
            "Emissions required."
        );
        require(address(_token) != address(token), "Disburse token.");
        require(
            _tokenStrength > 0 && _pointStrength > 0,
            "Strength/s are Zero."
        );
        ...
        if (address(poolInfo[_token].token) == address(0)) {
            ...
            totalTokenStrength =
                (totalTokenStrength - poolInfo[_token].tokenStrength) +
                _tokenStrength;
            poolInfo[_token].tokenStrength = _tokenStrength;
            totalPointStrength =
                (totalPointStrength - poolInfo[_token].pointStrength) +
                _pointStrength;
            ...
            uint256[] memory boosters = new uint256[](
                poolInfo[_token].boostInfo.length + _boostInfo.length
            );
  ...
  */
    function updatePool(IERC20 _token) internal {
        ...
        pool.pointsPerShare =
            pool.pointsPerShare +
            (pointsReward / poolInfo[_token].pointBoostedDeposit);
        pool.lastRewardEvent = block.timestamp;
    }
  ...
  */
    function stakeItemsBatch(
        ...
        if (ISuperGeneric(_contract).supportsInterface(INTERFACE_ERC721)) {
            ...
            ISuperGeneric(_contract).supportsInterface(INTERFACE_ERC1155)
        ) {
            ISuperGeneric(_contract).safeBatchTransferFrom(
                _msgSender(),
                address(this),
  ...
  */
    function unstakeItemsBatch(IERC20 _token, uint256 _boosterId)
        external
        nonReentrant
    {
        ...
        if (
            ISuperGeneric(externalContract).supportsInterface(INTERFACE_ERC721)
        ) {
            ...
            ISuperGeneric(externalContract).supportsInterface(INTERFACE_ERC1155)
        ) {
            ISuperGeneric(externalContract).safeBatchTransferFrom(
                address(this),
                _msgSender(),
  ...
  */
    function spendPoints(address _user, uint256 _amount) external {
        require(approvedPointSpenders[msg.sender], "Not allowed.");
        uint256 _userPoints = getAvailablePoints(_user);
        require(_userPoints >= _amount, "Invalid amount.");
