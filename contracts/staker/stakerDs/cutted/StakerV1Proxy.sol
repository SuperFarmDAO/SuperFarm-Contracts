...
/**
 * @title A diamond standard proxy storage for StakerV1
 * @author Qazawat Zirak
 * A proxy storage for a StakerV1. It is the main entry point for
 * contract calls. Every call to proxy storage will be delegated to
 * StakerV1 contract address. Storage is reflected in this contract.
 */
contract StakerV1Proxy {
    /**
     * Construct a new stakerV1 proxy.
    ...
    constructor(
        ...
        require(
            ...
            "StakerV1Proxy::Constructor: mismatch of arrays lengths."
        );
        ...
        require(success, "StakerV1Proxy::Constructor: Delegate call failed");

        // If deployment is success, store constructor parameters
        //b.IOUTokenAddress = _IOUTokenAddress;
        uint256 MAX_INT = 2**256 - 1;
        b.name = _name;
        b.token = address(_token);
        IERC20(b.token).approve(address(this), MAX_INT);
    ...
    fallback() external payable {
        ...
        require(
            ...
            "StakerV1Proxy::fallback: No implementation found"
        );
