...
contract Staker is Ownable, ReentrancyGuard {
  ...
  */
        ...
        external
    {
        ...
        require(developerShare > 0, "You are not a developer of this Staker.");
        require(
            _newShare <= developerShare,
  ...
  */
    function addPool(
        ...
        uint256 lastPointRewardTime = block.timestamp >
            earliestPointEmissionTime
            ? block.timestamp
            : earliestPointEmissionTime;
        uint256 lastRewardTime = lastTokenRewardTime > lastPointRewardTime
            ? lastTokenRewardTime
            : lastPointRewardTime;
  ...
  */
    function updatePool(IERC20 _token) internal {
        ...
        pool.tokensPerShare =
            pool.tokensPerShare +
            (tokensReward / poolTokenSupply);
        pool.pointsPerShare =
            pool.pointsPerShare +
            (pointsReward / poolTokenSupply);
        pool.lastRewardTime = block.timestamp;
    }
  ...
  */
    function spendPoints(address _user, uint256 _amount) external {
        require(
            approvedPointSpenders[msg.sender],
