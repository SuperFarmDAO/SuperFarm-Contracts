...
contract VestStream is Ownable, ReentrancyGuard {
  ...
  */
  function claimableAmount(address beneficiary) public view returns (uint256) {
    ...
    uint256 time = currentTimestamp - claim.startTime;
    uint256 claimPercent = (time * 1e18) / (claim.endTime - claim.startTime);
  ...
  */
  function claim(address beneficiary) external nonReentrant {
    ...
    uint256 time = currentTimestamp - _claim.startTime;
    uint256 claimPercent = (time * 1e18) / (_claim.endTime - _claim.startTime);
