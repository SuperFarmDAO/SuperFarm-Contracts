...
contract FacetMintFromPool is SuperMerkleAccessds, ReentrancyGuard {
  ...
  */
    function mintFromPool(
        ...
        require(
            _id < b.nextPoolId &&
                b.pools[_id].config.singlePurchaseLimit >= _amount,
            "0x1B"
        );
        ...
        require(
            (block.timestamp >= b.pools[_id].config.startTime &&
                block.timestamp <= b.pools[_id].config.endTime) || whiteListed,
            "0x4B"
        );
        ...
        require(_assetIndex < b.pools[_id].itemPricesLength[itemKey], "0x3B");
        ...
        uint256 userGlobalPurchaseAmount = _amount +
            b.globalPurchaseCounts[_msgSender()];

        if (b.globalPurchaseLimit != 0) {
            require(userGlobalPurchaseAmount <= b.globalPurchaseLimit, "0x5B");

            // Verify that the pool is respecting per-address pool purchase limits.
        }
        uint256 userPoolPurchaseAmount = _amount +
            b.pools[_id].purchaseCounts[_msgSender()];

        // Verify that the pool is not depleted by the user's purchase.
        uint256 newCirculatingTotal = b.pools[_id].itemMinted[itemKey] +
            _amount;
        require(newCirculatingTotal <= b.pools[_id].itemCaps[itemKey], "0x7B");
    ...
    function sellingHelper(
        ...
        if (_whiteListPrice) {
            SuperMerkleAccessds.AccessList
                storage accessList = SuperMerkleAccessds.accessRoots[
                    _accesListId
                ];
            uint256 price = accessList.price * _amount;
            if (accessList.token == address(0)) {
                require(msg.value >= price, "0x9B");
                (bool success, ) = payable(b.paymentReceiver).call{
                    value: msg.value
                }("");
                require(success, "0x0C");
                b.pools[_id].whiteLists[_accesListId].minted[
                    _msgSender()
                ] = true;
            } else {
                require(
            ...
            BlueprintSuperMintShop1155.Price storage sellingPair = b
                .pools[_id]
                .itemPrices[itemKey][_assetIndex];
            if (
                sellingPair.assetType ==
                BlueprintSuperMintShop1155.AssetType.Point
            ) {
                ...
                sellingPair.assetType ==
                BlueprintSuperMintShop1155.AssetType.Ether
            ) {
                ...
                sellingPair.assetType ==
                BlueprintSuperMintShop1155.AssetType.Token
            ) {
    ...
    function checkRequirments(uint256 _id) private view returns (bool) {
        ...
        BlueprintSuperMintShop1155.PoolRequirement memory poolRequirement = b
            .pools[_id]
            .config
            .requirement;
        if (
            poolRequirement.requiredType ==
            BlueprintSuperMintShop1155.AccessType.TokenRequired
        ) {
            // bytes data
            ...
            poolRequirement.requiredType ==
            BlueprintSuperMintShop1155.AccessType.PointRequired
        ) {
        ...
        if (poolRequirement.requiredId.length == 0) {
            if (
                poolRequirement.requiredType ==
                BlueprintSuperMintShop1155.AccessType.ItemRequired
            ) {
                for (
                    uint256 i = 0;
                    ...
                    amount += ISuper1155(poolRequirement.requiredAsset[i])
                        .totalBalances(_msgSender());
                }
                return amount >= poolRequirement.requiredAmount;
            } else if (
                poolRequirement.requiredType ==
                BlueprintSuperMintShop1155.AccessType.ItemRequired721
            ) {
                for (
                    uint256 i = 0;
                    i < poolRequirement.requiredAsset.length;
