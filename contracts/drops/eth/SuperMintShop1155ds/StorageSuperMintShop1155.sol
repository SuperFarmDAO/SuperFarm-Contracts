...
contract StorageSuperMintShop1155 {
    ...
    */
    constructor(address _linkProxy, address _paymentReceiver, uint256 _globalPurchaseLimit, uint256 _maxAllocation) {
        ...
        bytes32 linkProxyKey = keccak256(abi.encodePacked(selector, baseContractName));

        // Do a delegate call to initialize the owner
        address initializerFacet = ILinkProxy(_linkProxy).links(linkProxyKey);
        require(initializerFacet != address(0), "StorageSuperMintShop1155: No facet found!");
