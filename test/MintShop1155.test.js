...
describe("===MintShop1155, PermitControl, Sweepable===", function () {
  ...
  beforeEach(async function () {
    ...
    //item721 = await this.Item721.deploy();
    ...
    staker = await this.Staker.deploy("firstStaker", mockERC20.address);
    let whiteList = {
    ...
    await mintShop1155
      .connect(owner)
      .setItems([super1155.address, super1155Second.address]);
  ...
  describe("Constructor", function () {
    it("should initialize values as expected", async function () {
      ...
      expect(await mintShop1155.paymentReceiver()).to.equal(
        paymentReceiver.address
      );
    ...
    it("should deploy a new instance where deployer is the owner", async function () {
      ...
      expect(await mintShop1155v2.paymentReceiver()).to.equal(
        paymentReceiver.address
      );
  ...
  describe("updatePaymentReceiver", function () {
    ...
    it("Reverts: setting manager for Zero address", async function () {
      ...
      await expect(
        mintShop1155
          ...
          .setManagerRight(
            zeroRight,
            ethers.utils.defaultAbiCoder.encode(input.types, input.values)
          )
    ...
    it("should test PermitControl to set a manager who can further create permits to update payment receiver", async function () {
      ...
      await mintShop1155
        .connect(owner)
        .setManagerRight(setPaymentReceiverRight, MANAGER);
      ...
      await mintShop1155
        .connect(signer1)
        .updatePaymentReceiver(paymentReceiver.address);
      expect(await mintShop1155.paymentReceiver()).to.equal(
        paymentReceiver.address
      );
    });
  ...
  describe("updatePool, addPool, getPools", function () {
    ...
    it("Reverts: updatePool a non-existent pool", async function () {
      ...
      let latestBlock = await ethers.provider.getBlock(
        await ethers.provider.getBlockNumber()
      );
      await expect(
        mintShop1155.connect(owner).updatePool(
          1,
          {
            name: "firstPool",
    ...
    it("should add a pool, update it, get the pool", async function () {
      ...
      latestBlock = await ethers.provider.getBlock(
        await ethers.provider.getBlockNumber()
      );
      await mintShop1155.connect(owner).updatePool(
        0,
        {
          name: "firstPool",
          startTime: latestBlock.timestamp,
  ...
  describe("mintFromPool, getPoolsWithAddress, getPurchaseCounts", function () {
    ...
    it("Reverts: mintFromPool amount less than 0", async function () {
      ...
      let whiteListInput = {
        ...
        node: hash(
          getIndex(whiteList, signer1.address),
          signer1.address,
          whiteList[signer1.address]
        ),
        merkleProof: computeMerkleProof(
          getIndex(whiteList, signer1.address),
          whiteList
        ),
      };
    ...
    it("Reverts: mintFromPool amount greater than singlePurchaseLimit", async function () {
      ...
      await expect(
        mintShop1155
          .connect(deployer)
          .mintFromPool(0, 2, 1, 3, 0, whiteListInput)
      ).to.be.revertedWith("0x1B");
    });
    ...
    it("Reverts: mintFromPool assetindex not valid", async function () {
      ...
      await expect(
        mintShop1155
          .connect(signer1)
          .mintFromPool(0, 2, 5, 1, 0, whiteListInput)
    ...
    it("Reverts: mintFromPool pool is not running", async function () {
      ...
      await expect(
        mintShop1155
          .connect(signer1)
          .mintFromPool(0, 2, 0, 1, 0, whiteListInput, {
    ...
    it("Reverts: mintFromPool not enough items available for purchase", async function () {
      ...
      await mintShop1155
        .connect(signer1)
        .mintFromPool(1, 3, 0, 1, 0, whiteListInput, {
          value: ethers.utils.parseEther("1"),
        });
    ...
    it("Reverts => Success: mintFromPool not enough ERC20 tokens for the pool then getPrchaseCount", async function () {
      ...
      await mockERC20
        .connect(deployer)
        .transfer(signer1.address, ethers.utils.parseEther("5"));
      ...
      await mockERC20
        .connect(signer1)
        .approve(mintShop1155.address, ethers.utils.parseEther("15"));

      await expect(
        mintShop1155
          .connect(signer1)
          .mintFromPool(1, 3, 1, 3, 0, whiteListInput)
      ...
      await mintShop1155
        .connect(signer1)
        .mintFromPool(1, 3, 1, 1, 0, whiteListInput);

      // getPurchaseCounts
      let balances = await mintShop1155
        .connect(signer1)
        .getPurchaseCounts(
          [signer1.address, signer2.address, owner.address],
          [1]
        );
    ...
    it("Reverts: not enough ERC1155 required items", async function () {
      ...
      await super1155
        .connect(owner)
        .mintBatch(signer1.address, [shiftedItemGroupId], ["1"], DATA);
  ...
  describe("sweep, lockSweep", function () {
    ...
    it("should sweep tokens back from mintshop1155 to the user address", async function () {
      await mockERC20
        .connect(deployer)
        .transfer(signer3.address, ethers.utils.parseEther("10"));

      await mockERC20
        .connect(signer3)
        .transfer(mintShop1155.address, ethers.utils.parseEther("5"));
      ...
      await expect(await mockERC20.balanceOf(signer3.address)).to.be.equal(
        ethers.utils.parseEther("10")
      );
    });
  });
  ...
  describe("max allocation test", function () {
    it("Shoud revert", async function () {
      ...
      await mShop.connect(signer1).mintFromPool(0, 1, 0, 1, 0, whiteListInput, {
        value: ethers.utils.parseEther("1"),
      });
      await expect(
        mShop.connect(signer1).mintFromPool(0, 1, 0, 1, 0, whiteListInput, {
          value: ethers.utils.parseEther("1"),
        })
  ...
  describe("mintFromPool with whiteList ristrictions", function () {
    ...
    it("Shoud revert, user already bought", async function () {
      ...
      await mintShop1155
        .connect(signer1)
        .mintFromPool(0, 2, 1, 1, 0, whiteListInput, {
    ...
    it("Shoud revert, user not in whiteList", async function () {
      ...
      await expect(
        mintShop1155
          .connect(owner)
          .mintFromPool(1, 2, 1, 1, 0, whiteListInput, {
            value: ethers.utils.parseEther("1"),
          })
      ).to.be.revertedWith("0x4B");
    });
