...
describe('ContractFactory', function () {
	...
	describe('testing the contract factory', function () {
		...
		// Verify that contract deployment is possible.
		it('contract deployment produces addresses as expected', async () => {

			// To prevent front-running, the salt encodes the intended sender.
			const salt = `${alice.address}000000000000000000000001`;

			// This is the bytecode for a payable-construction Counter contract.
			const bytecode = '';
		...
		it('contract deployment fails if value sent to non-payable', async () => {

			// To prevent front-running, the salt encodes the intended sender.
			const salt = `${alice.address}000000000000000000000001`;

			// This is the bytecode for a nonpayable-construction Counter contract.
			...
			// Verify that contract creation fails with a helpful error.
			await expect(
				contractFactory.connect(alice.signer).deploy(bytecode, salt, {
					value: ethers.utils.parseEther('0.1')
				})
			).to.be.revertedWith('CreationFailedCheckConstructor()');
		});
	});
});
