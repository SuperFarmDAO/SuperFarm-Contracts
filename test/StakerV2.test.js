...
describe("===Stakerv2===", function () {
  ...
  let rewardToken, depositToken, stakerv2, super721, super1155, proxyRegistry;
  ...
  describe("All", function () {
    it("stakerv2", async function () {
      ...
      await stakerv2.connect(owner).configureBoostersBatch(
        [0, 1],
        [
          {
            set: true,
      ...
      await stakerv2
        .connect(owner)
        .addPool(depositToken.address, 10000, 10000, [0, 1]);
      await depositToken
        .connect(signer1)
        .approve(stakerv2.address, ethers.utils.parseEther("1000"));
      await depositToken
        .connect(signer2)
        .approve(stakerv2.address, ethers.utils.parseEther("1000"));
      ...
      await super1155
        .connect(signer2)
        .setApprovalForAll(stakerv2.address, true);

      //User1-Deposit
      await stakerv2
        .connect(signer1)
        .deposit(depositToken.address, ethers.utils.parseEther("200"));
      ...
      await stakerv2
        .connect(signer2)
        .deposit(depositToken.address, ethers.utils.parseEther("150"));
      await network.provider.send("evm_increaseTime", [15]);
      ...
      await stakerv2
        .connect(signer1)
        .unstakeItemsBatch(depositToken.address, 0);
      await network.provider.send("evm_increaseTime", [15]);

      //User2-Withdraw
      await stakerv2
        .connect(signer2)
        .withdraw(depositToken.address, ethers.utils.parseEther("100"));
      await network.provider.send("evm_increaseTime", [15]);
      ...
      await stakerv2
        .connect(signer2)
        .withdraw(depositToken.address, ethers.utils.parseEther("50"));
      await stakerv2
        .connect(signer1)
        .withdraw(depositToken.address, ethers.utils.parseEther("350"));
      await stakerv2.connect(signer1).claim(depositToken.address);
      await stakerv2.connect(signer2).claim(depositToken.address);
      // console.log(
      ...
      // );
      // console.log(
      //   await (await rewardToken.balanceOf(signer2.address)).toString()
      // );
      // console.log(
      //   await stakerv2.connect(signer3).getItemsUserInfo(signer2.address, 1)
      // );
